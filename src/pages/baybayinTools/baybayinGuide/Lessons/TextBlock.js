...
const TextBlock = ({ block, showLine = false, fontSize = "15px" }) => {
  ...
  const renderTextWithBoldAndBaybayin = () => {
    ...
    // Escape special characters in bold words for regex
    const escapeRegex = (str) => str.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

    // Split and apply bold words
    let content = [text];
    boldWords.forEach((word) => {
      content = content.flatMap((fragment, index) =>
        typeof fragment === "string"
          ? fragment.split(new RegExp(`(${escapeRegex(word)})`, "gi")).map((part, partIndex) =>
    ...
    // Apply Baybayin font and mapping
    content = content.flatMap((fragment, index) =>
      typeof fragment === "string"
